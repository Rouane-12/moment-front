...
const OUTGOING_COOLDOWN = 2000;
let lastCallEndTime = 0;
const CALL_END_COOLDOWN = 500; // reduced from 2000 to 500ms — was causing blocked calls
...
function GlobalCallListenerInner() {
  ...
  const endCall = useCallback((sendMissed = false) => {
    ...
    if (sendMissed && dir === "outgoing" && peer) {
      const apiBase = import.meta.env["VITE_API_URL"] || "http://localhost:5200";
      ...
      }).catch((e) => console.error("📞 Missed call message failed:", e));
    }
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      console.log("📞 Socket connected:", socket.id, "transport:", (socket.io as any).engine?.transport?.name);
    });
    ...
    const handleParentIce = (data: any) => {
      if (phaseRef.current !== "none") {
        console.log("📞 Parent buffering call-ice-candidate, phase:", phaseRef.current, "buffer size:", bufferedIce.current.length + 1);
        bufferedIce.current.push(data);
      } else {
        console.log("📞 DROPPED call-ice-candidate — phase is none (no active call)");
      }
    };
    ...
    socket.on("call-init", (data: any) => {
      ...
      console.log("📞 INCOMING call-init from:", data.from?.firstName, "dedupeKey:", dedupeKey, "phase:", phaseRef.current, "cooldown:", now - lastCallEndTime, "socket connected:", socket.connected);
      ...
      if (phaseRef.current !== "none") {
        ...
        if (socket.connected && data.from?._id) {
          socket.emit("call-end", { to: data.from._id, callId: data.callId || "busy", from: user?.id });
        }
        return;
      }
      if (now - lastCallEndTime < CALL_END_COOLDOWN) {
        console.log("📞 IGNORED — cooldown active:", CALL_END_COOLDOWN - (now - lastCallEndTime), "ms remaining");
        if (socket.connected && data.from?._id) {
          socket.emit("call-end", { to: data.from._id, callId: data.callId || "busy", from: user?.id });
        }
        return;
      ...
      console.log("📞 NEW incoming call from:", data.from?.firstName, data.from?.lastName);

      // Show notification
      if ("Notification" in window && Notification.permission === "granted") {
        try {
          new Notification(`Appel entrant de ${data.from?.firstName} ${data.from?.lastName}`, {
    ...
    const handler = (e: CustomEvent) => {
      if (
        ...
        if (Date.now() - lastOutgoingCallTime < OUTGOING_COOLDOWN) {
          console.log("📞 Outgoing call cooldown — skipping");
          return;
        }
        ...
        bufferedIce.current = [];
        bufferedAnswer.current = [];
        console.log("📞 START OUTGOING to:", e.detail.targetUser.firstName, e.detail.targetUser.lastName);
  ...
  const rejectCall = useCallback(() => {
    ...
    if (call && socketRef.current?.connected) {
      socketRef.current.emit("call-end", { to: call.from._id, callId: "reject", from: user?.id });
    }
    lastCallEndTime = Date.now();
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/95 flex flex-col items-center justify-center">
      ...
      <div className="flex gap-4">
        <button onClick={onAccept} className="w-16 h-16 rounded-full bg-green-500 hover:bg-green-600 transition-colors flex items-center justify-center">
          <Phone className="w-8 h-8 text-white" />
        </button>
        <button onClick={onReject} className="w-16 h-16 rounded-full bg-red-500 hover:bg-red-600 transition-colors flex items-center justify-center">
...
function ActiveCallOverlay({
  ...
  const drainBuffer = useCallback(async () => {
    ...
    if (!pc) {
      ...
      if (total > 0) {
        console.log("📞 drainBuffer: peer not ready, buffers:", answerBuffer.current.length, "answers,", iceBuffer.current.length, "ICE");
      }
      return;
    }

    // 1) Apply buffered answer FIRST (sets remoteDescription so ICE can be added)
    ...
    // 2) Apply buffered ICE candidates (only possible after remoteDescription is set)
    let appliedAny = true;
    while (appliedAny && iceBuffer.current.length > 0 && peerRef.current && !unmountedRef.current) {
      appliedAny = false;
      const data = iceBuffer.current.shift()!;
      try {
        await peerRef.current.addIceCandidate(new RTCIceCandidate(data.candidate));
        console.log("📞 Applied buffered ICE candidate, remaining:", iceBuffer.current.length);
        appliedAny = true;
      } catch (e: any) {
        if (e?.name === "InvalidStateError") {
          // Remote description not set yet — re-queue
          iceBuffer.current.push(data);
          console.log("📞 ICE re-queued (remote desc not ready), remaining:", iceBuffer.current.length);
          break;
        }
  ...
  useEffect(() => {
    ...
    console.log("📞 Subscribing to call-answer and call-ice-candidate on socket, connected:", s.connected);

    const handleAnswer = (data: any) => {
      console.log("📞 call-answer received, peer ready:", !!peerRef.current, "unmounted:", unmountedRef.current);
      if (unmountedRef.current) return;
      if (data.answer) {
    ...
    const handleIce = (data: any) => {
      ...
      if (data.candidate) {
        iceBuffer.current.push(data);
        console.log("📞 ICE candidate queued, peer ready:", !!peerRef.current, "buffer:", iceBuffer.current.length);
        drainBuffer();
      }
    };
  ...
  // Periodic drain — keeps trying to apply buffered data
  useEffect(() => {
    ...
    (targetId: string, stream: MediaStream) => {
      ...
      pc.onicecandidate = (event) => {
        if (event.candidate && !unmountedRef.current) {
          const s = parentSocketRef;
          if (s?.connected) {
            const candidateStr = event.candidate.candidate?.substring(0, 60) || "";
            console.log("📞 Sending ICE to:", targetId, candidateStr);
            ...
            console.log("📞 Socket NOT connected — ICE candidate LOST");
          }
      ...
      pc.onicecandidateerror = (event: any) => {
        // Don't log STUN errors too loudly — they're expected on mobile
        if (event.url?.includes("stun")) {
          console.log("📞 ICE STUN error (expected on mobile):", event.errorCode);
        } else {
      ...
      pc.ontrack = (event) => {
        ...
        if (!unmountedRef.current && remoteAudioRef.current && event.streams[0]) {
      ...
      pc.onconnectionstatechange = () => {
        ...
        if (state === "failed") {
          console.log("📞 WebRTC connection failed — will retry via ICE restart if possible");
        }
        if (state === "closed") {
          cleanup();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        console.log("📞 Getting media for incoming call from:", incomingCall.from.firstName);
        const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        if (aborted || unmountedRef.current) {
          stream.getTracks().forEach((t) => t.stop());
          return;
        ...
        const s = parentSocketRef;
        if (!s?.connected) {
          console.error("📞 Socket not connected for answer, aborting");
          return;
        }
        console.log("📞 Emitting call-answer to:", incomingCall.from._id);
        s.emit("call-answer", { to: incomingCall.from._id, answer, from: user?.id });
    ...
    if (status === "connecting") {
      const timeout = setTimeout(() => {
        if (!unmountedRef.current && statusRef.current === "connecting") {
          console.log("📞 Call timeout (45s) — peer:", !!peerRef.current, "iceBuf:", iceBuffer.current.length, "answerBuf:", answerBuffer.current.length);
          cleanup();
          onEndRef.current(false);
        }
      }, 45000);
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/95 flex flex-col items-center justify-center">
      ...
      <div className="flex items-center gap-6 mt-12">
        ...
        <button onClick={endCall} className="w-16 h-16 rounded-full bg-red-500 hover:bg-red-600 flex items-center justify-center transition-colors hover:scale-105">
